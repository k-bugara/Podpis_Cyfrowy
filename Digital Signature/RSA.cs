...
namespace Digital_Signature
{
    public class RSA
    {
        ...
        #region Methods
        #region public BigInteger EncryptHash(BigInteger hash). Zaszyfruj wartość skrótu za pomocą tajnego klucza RSA.
        public BigInteger EncryptHash(BigInteger hash)
        {
            BigInteger EulersFuncOfR = (P - 1) * (Q - 1); // P i Q to liczby pierwsze

            if (D == 0)
            {
                if (GCD(E, EulersFuncOfR) != 1)
                {
                    MessageBox.Show("Error: Największy wspólny dzielnik E i funkcja Eulera z R musi być równa 1.");
                    return 0;
                }
        ...
        #region private BigInteger ModPower(BigInteger a, BigInteger z, BigInteger n). Szybkie potęgowanie modułowe dla liczb BigInteger. x = a^z mod n
        ...
        #endregion
        #region private static BigInteger GCD(BigInteger a, BigInteger b). Największy wspólny dzielnik
        ...
        #endregion
        #region private BigInteger Euclide(BigInteger a, BigInteger b). Oblicz gcd(a, b) i x, y, gdzie ax + by = gcd(a, b). Zwróć y.
